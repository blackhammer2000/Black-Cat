...
window.onload = () => {
  const packsSectionBody = document.querySelectorAll(
    "[data-packs-category-body]"
  );
  packsSectionBody.forEach((div) => {
    renderProducts(products.wholesale.reverse(), div);
  });
};
...
function createProductCard(item, index) {
  ...
  product.style.transform = `translateX(400px) scale(0.1)`;
