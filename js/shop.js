...
window.onload = () => {
  const recommendedBody = document.querySelector(
    "[data-recommended-category-body]"
  );
  const newRecipeBody = document.querySelector(
    "[data-new-recipe-category-body]"
  );
  const favoriteBody = document.querySelector("[data-favorite-category-body]");
  const saverBody = document.querySelector("[data-saver-category-body]");

  // console.log(recommendedBody, newArrivalsBody);

  renderProducts(products.retail, saverBody);
  renderProducts(products.wholesale, favoriteBody);
  renderProducts(products.retail.reverse(), recommendedBody);
...
function createProductCard(item, index, body) {
  ...
  product.style.transition = `animation ${(index + 1) / 10}s ease-in-out`;
